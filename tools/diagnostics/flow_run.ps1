...
#
# ⛔⛔ THE RULE ABOVE HAD NOT BEEN FOLLOWED 62 TIMES (measured 2026-09-06, effects-path 1:1 audit).
# The engine reads 134 distinct BRN_* variables; this list plus the teleport/sweep wipe below
# covered 71 of them, and the four in the "INHERITED diag env" block are deliberately inherited AND
# RECORDED. That left SIXTY-TWO engine variables that no wipe touched and no line reported, so a
# leftover `$env:BRN_X=1` from any earlier command in the same shell rode silently into every
# subsequent run that called itself DEFAULT. They are added below. Several are not instruments at
# all but BEHAVIOUR CONTROLS, which is why this is not cosmetic:
#   BRN_SKID_DISABLE      forces visualfxsurface's SkidMarksEnabled false -> NO TYRE MARKS ANYWHERE
#   BRN_LIONFX_NOCULL / BRN_LION_NOBLEND      change what the particle pass draws and how
#   BRN_TDEF_NO_UPLOAD / BRN_TDEF_SCALE       change traffic deformation on the render side
#   BRN_WORLD_ONLY / BRN_ENVMAP_ALLFACES / BRN_ENVMAP_30HZ    change what is rendered at all
#   BRN_FORCE_TAKEDOWN / BRN_AI_MADNESS / BRN_PROP_NOCLAMP / BRN_STRENGTH_STAT_OVERRIDE /
#   BRN_SYMP_LATCH_CONTROL / BRN_TRAFFIC_EVENTS_CONTROL / BRN_SKID_LIFT   change the SIMULATION
# A wave measuring "the marks are gone" through a stale BRN_SKID_DISABLE would have blamed the
# renderer. Pass any of them THROUGH the wipe with -DiagEnv, which is applied after it.
...
foreach ($v in @('BRN_RC_PROBE','BRN_DIRECTOR_TRACE','BRN_FORCE_DIRECTOR_CAMERA','BRN_WORLD_CAMFREE','BRN_MOTION_PROBE','BRN_TRICACHE_PROBE','BRN_TRACTION_PROBE','BRN_CRASH_PLAYER','BRN_START_EVENT','BRN_AI_DRIVES_PLAYER','BRN_BOOST_TICKER_DIAG','BRN_DEBUG_FINISH_POS','BRN_DEBUG_FINISH_AT','BRN_START_SHOWTIME','BRN_SHOWTIME_WATCH','BRN_DEFORM_TRACE','BRN_SKIP_TRAINING_TIP','BRN_EVENT_FSM','BRN_APT_LIFE','BRN_ASSERT_NO_SUPPRESS','BRN_CRASHCAM_DIAG','BRN_CULL_OFF','BRN_DOF_TRACE','BRN_DRIVETHRU_DIAG','BRN_EFFECTS_DIAG','BRN_ENGINE_PROBE','BRN_ENVMAP_DEBUG','BRN_GESTURE_DIAG','BRN_ICE_TIMESCALE_DIAG','BRN_ICE_TRACE','BRN_IOBUF_ZERO','BRN_JUNCTION_DIAG','BRN_MODEMGR_DIAG','BRN_POSTFX_CALIBRATION_TEST','BRN_POSTFX_CALIB_SCREEN_TEST','BRN_QUEUE_WATERMARK','BRN_SHADOW_BIAS','BRN_SHADOW_CULL','BRN_SHADOW_FALLBACKVS','BRN_SHADOW_FORCECWE','BRN_SHADOW_SLOPEBIAS','BRN_SHADOW_ZALWAYS','BRN_SLOMO_DIAG','BRN_SLOMO_LATCH_SKIP','BRN_TRAFFIC_DIAG','BRN_TRAFFIC_FAKE_SHOWTIME','BRN_TRAFFIC_NO_JAM_NUKE','BRN_SHOWTIME_IGNORE_PROGRESSION','BRN_TYRE_PROBE','BRN_WALL_PROBE','BRN_WHEEL_DIAG','BRN_WHEEL_LOCK_DIAG','BRN_WHEEL_ZALWAYS','BRN_FRAME_DUMP_ARM','BRN_FRAME_DUMP_MAX','BRN_LION_WHITE_PIN','BRN_LION_QRES_OFF','BRN_LION_QRES_SHOW','BRN_LION_QRES_ADD','BRN_CRUMPLE_PROBE','BRN_CRUMPLE_FORCE','BRN_SCRATCH_PROBE','BRN_SCRATCH_FORCE',
                  ...
                  'BRN_RESTROW_PROBE','BRN_ROLL_PROBE','BRN_RT_PROBE','BRN_SATNAV_DIAG','BRN_JUMP_DIAG','BRN_SCENE_QUERY_DIAG','BRN_SHOWTIME_SCORE_DIAG',

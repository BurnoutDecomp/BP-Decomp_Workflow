# takedown_organic_weave -- SAME CHAIN AS takedown_organic, BUT THE PLAYER IS AIMED AT THE PACK.
#
# Run it:  powershell -ExecutionPolicy Bypass -File tools\tests\run_case.ps1 -Case takedown_organic_weave
#
...
#
# THE APPROACH. -SteerScript sweeps the car across the road through exactly that window: one right
# pulse into slots 1/2 as they come up the right side, then four left pulses through the slot 5 /
# slot 3 / slot 4 passes on the left. Nothing else changes -- same teleport, same event start, same
# throttle hold, same knobs. A pulse is full lock (the token vocabulary is none|left|right), so
# each one is kept short; the point is to sweep lateral position across the pack's line, not to
# turn the car around.
#
...
@{
  ...
  Run     = @{
    ...
    SteerScript     = '0:none,5.0:right,6.3:none,7.4:left,8.4:none,9.2:left,10.1:none,10.6:left,11.5:none,12.1:left,13.1:none'
    MaxSeconds      = 110
  }

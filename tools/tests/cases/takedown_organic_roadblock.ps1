# takedown_organic_roadblock -- SAME CHAIN AS takedown_organic, BUT THE PLAYER IS AIMED AT THE PACK.
#
# Run it:  powershell -ExecutionPolicy Bypass -File tools\tests\run_case.ps1 -Case takedown_organic_roadblock
#
...
#
# WHAT THE FOUR STEERING VARIANTS MEASURED, AND WHY THIS ONE STOPS STEERING.
#
...
#
# THREE MECHANICS CAME OUT OF THOSE, ALL FROM THE [motion] steer/x columns:
#   * The start-line clamp pins the STEERING as well as the throttle. steer reads exactly 0.000000
#     for the whole clamp and snaps to the pi/8 limit on the single frame gas goes 0.0999 -> 1.0
#     (n=1470, about t+8.1). A token pressed after that mostly never lands, so only the token held
#     at t+0 is reliable.
#   * The signs are real and opposite: `right` took the car to +x (3044.6 -> 3056.8), `left` to -x
#     (3044.6 -> 3031.0), which is the pack's side.
#   * FULL LOCK IS A SPIN, NOT A LANE CHANGE. Both locks cost the car all its speed within ~2 s
#     (leftcut fell from 11.9 m/s to 0.84 m/s) and parked it off the racing line, so the player
#     arrived in the pack's lane about 4 m wide of it and 20-55 m up-road of the passes. The
#     vocabulary has no partial lock, so steering cannot make this approach.
#
# THE APPROACH. Stop trying to aim a spinning car and become the obstacle instead. `brake` held
# from t+0 -- the throttle channel, so the steering stays neutral and the car stays in its own
# lane at x 3044.6 -- kills the run-away the moment the clamp releases and leaves the player
# standing on the racing line. The rivals demonstrably drive through that line: leftcut caught
# rival 1 at x 3045.0 and rival 3 at x 3044.0 crossing the player's own z, and every run has them
# looping back through this stretch at 13-53 m/s for the rest of the window. This is the fallback
# the brief allows -- a rival-on-player contact rather than a player-on-rival ram -- and it is
# aimed squarely at the leg that has never fired: [td-contact], and with it the ownerB histogram
# on a pair that is actually two race cars.
#
...
@{
  Name    = 'takedown_organic_roadblock'
  Area    = 'takedown'

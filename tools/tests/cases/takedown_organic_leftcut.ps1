# takedown_organic_leftcut -- SAME CHAIN AS takedown_organic, BUT THE PLAYER IS AIMED AT THE PACK.
#
# Run it:  powershell -ExecutionPolicy Bypass -File tools\tests\run_case.ps1 -Case takedown_organic_leftcut
#
...
#
# WHAT THE THREE EARLIER SWEEPS MEASURED, AND THE ONE MECHANIC THAT MATTERS.
#
#   takedown_organic_weave    20260913_160142  five left/right pulses over t+5..t+13  min 7.18 m
#   takedown_organic_cutin    20260913_160627  `right` held t+9.8..12.4 and 12.9..14.2 min 7.04 m
#   takedown_organic_rightlock 20260913_161001 `right` held from t+0 to t+16          min 3.37 m
#
# THE MECHANIC. The start-line clamp does not only pin the throttle at ~0.10 and the speed at
# 8.05 m/s -- it pins the STEERING TOO. In every run the [motion] steer column reads exactly
# 0.000000 for the whole clamp, and in rightlock it snaps to full lock on the single frame the
# clamp lets go (n=1470: gas 0.0999 -> 1.0, steer 0.0 -> -0.296 -> -0.475 -> -0.3927 = pi/8, the
# limit). A lock that is ALREADY HELD when the clamp releases therefore lands on the first free
# frame; the two runs that first pressed a steer token after the clamp had gone (t+9.8 in cutin,
# t+7.5 and t+9.3 in weave) were mostly ignored, so the reliable recipe is to set the token at t+0
# and hold it. rightlock also proved the tokens do reach the car -- it drove the player off the
# road into a tight loop at (3056, -1968) and parked it there for the rest of the run, which is
# why its 3.37 m closest approach came from rivals passing a stationary player.
#
# THE APPROACH. Hold `left` from t+0 -- the mirror of rightlock, and the direction that aims at
# the pack. The rivals' line through this stretch is x ~ 3035-3037 while the player's start lane is
# x 3044.6, so a lock that is live from the clamp release at t+8.1 crosses ~8 m of road in about
# 1.5 s (rightlock measured 4.3 m in the first 0.8 s of lock) and puts the player physically in
# their lane for the slot 5 (t+8), slot 3 (t+10) and slot 4 (t+11) passes, all of which come
# through at 48-54 m/s. Release at t+10.2 so the car straightens instead of looping the way
# rightlock did, and run a long tail: the same traces show rivals returning to within 3.4-3.7 m
# later in the run once they have slowed to 13-15 m/s.
#
# If the car swings to +x again rather than -x, that is itself the finding -- both steer tokens
# would be reaching the car as the same direction -- and the [motion] x column says so in one run.
#

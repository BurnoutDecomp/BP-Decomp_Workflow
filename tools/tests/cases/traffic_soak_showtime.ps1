# traffic_soak_showtime -- lane traffic_assert. "traffic cars crashing the game with asserts, sometimes".
#
...
#
# SCENARIO 3 -- THE SHOWTIME WINDOW. Same lane soak, plus the both-bumpers showtime gesture at
# t+40 s. Showtime is what DEMOTES physical traffic wholesale (upstream 7761247a fixed an AV on
# exactly that route), so it is the scenario that exercises the demote half of the lifecycle --
# the half that leaves the world flag and the physics slot map disagreeing.
#
#
...
# Run it:   powershell -ExecutionPolicy Bypass -File tools\tests\run_case.ps1 -Case traffic_soak_showtime -ExpectFail -Label pre-fix
#

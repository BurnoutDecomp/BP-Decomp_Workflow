...
- **RenderWare & Vendor SDKs (EATech, rwcore, etc.): Test before decompiling.**
  For `EABase`, `EASTL`, and `EAThread`, we have the original open-source code in
  `vendor/`, so those original bodies do not need to be decompiled. If `work next` assigns a
  vendor SDK TU, run `python tools/work/check_vendor_lib.py <tu_name>`.
  - If the script says **PRESENT**: original buildable source exists; skip and block it
    (`work block <tu> "Vendor code; original source is present."`).
  - If the script says **MISSING**: reconstruct it from ARTIST like normal, or provide a
    documented platform leaf when the behavior is genuinely host-specific.
  - **PRESENT blocks the SDK's *bodies*, not its *types*.** You still **recover its public
...
- **RenderWare 4 follows the Paradise build, not a later host revision.** ARTIST is the
  authority for RenderWare behavior **and game-facing structure**: class relationships,
  vtable order, resource-lane count, and serialized contracts. DecFIGS supplies declaration
  names and shapes gated on ARTIST evidence. The PC implementation may widen pointers and
  use native host services, but it must not import structural drift from a different revision.
  Isolate genuine platform behavior with the original compiler flags, especially
  `D_PLATFORM_X360`. The canonical vocabulary lives in
  [`b5-decomp/vendor/renderware/include/rw/`](b5-decomp/vendor/renderware/include/rw/);
  maintain it by hand from the source-of-truth ladder and prove host sizes with
  `static_assert` under `RW_VERIFY_LAYOUT`.
- **`rw::audio::core::` types come from `IDA Files/ProStreet08Milestone.pdb`.** The EA
  ...
  caveat:** this is **X360 (32-bit pointers, big-endian)** —
...
- **Reconstruction Quality & Type Recovery (CRITICAL):** The goal is to reconstruct what the original C++ source code **looked like**, not to translate raw decompiler outputs literally.
  - **NO RAW OFFSET POINTER HACKS:** You must NEVER access member variables using raw offset casting (e.g., `*reinterpret_cast<Type*>(lThis + offset)` or `*(int*)(this + offset)`) or offset helper lambdas (like `Word(offset)`).
    - **Exception — external serialised / platform data.** Raw offset access *is* allowed
      ...
      follow the RenderWare reconstruction rule above).
